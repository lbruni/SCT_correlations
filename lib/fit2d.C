...
void fit2d()
{
   ...
   //gStyle->SetOptFit();
    ...
    TH2F *h_prova = new TH2F("h_prova","h_prova",100, -4,-1.0, 100, 6,0.06);
    
    Double_t x[100], y[100];
    for(int i = 0 ; i <100 ;i++)
        for(int j = 0 ; j <100 ;j++)
    {
        x[i] = histo1->GetXAxis()->GetBinCenter[j]*p1_1;
        y[i] =  histo1->GetYAxis()->GetBinCenter[j];
        h_prova->Fill(x[i],y[i]);
    }
    
    TCanvas *cprova = new TCanvas("cprova","show profile",900,1200);
    h_prova->Draw("colz");
    ...
    sct_analyis::Draw_New_aligment(hist1, f1);
